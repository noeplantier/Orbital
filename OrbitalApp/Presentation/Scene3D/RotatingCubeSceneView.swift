...
struct RotatingCubeSceneView: UIViewRepresentable {
    ...
    func updateUIView(_ scnView: SCNView, context: Context) {
        guard let cubeNode = scnView.scene?.rootNode.childNode(withName: "orbitalCube", recursively: true) else { return }
        ...
        if context.coordinator.lastResetTick != resetCameraTick,
           let cameraNode = scnView.scene?.rootNode.childNode(withName: "orbitalCamera", recursively: true) {
    ...
    private func makeScene() -> SCNScene {
        ...
        cubeNode.name = "orbitalCube"
        scene.rootNode.addChildNode(cubeNode)
        addSpinAction(to: cubeNode)

        let cameraNode = SCNNode()
        cameraNode.name = "orbitalCamera"
