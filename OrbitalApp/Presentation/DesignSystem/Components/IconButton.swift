...
struct IconButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: systemImage)
                ...
                .overlay(
                    Circle().strokeBorder(Color.orbitalBorder, lineWidth: style == .subtle ? 1 : 0)
                )
        }
    ...
    private var background: Color {
        switch style {
        case .subtle: return .orbitalSurfaceElevated
        case .prominent: return .orbitalPrimary
        case .danger: return .orbitalDanger
        }
    }

    private var foreground: Color {
        switch style {
        case .subtle: return .orbitalTextPrimary
