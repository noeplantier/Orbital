...
private struct AppTextFieldStyle: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        .frame(minHeight: 50)
        .background(Color.orbitalSurface, in: RoundedRectangle(cornerRadius: Radius.control, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: Radius.control, style: .continuous)
                .strokeBorder(isFocused ? Color.orbitalPrimary : Color.orbitalBorder, lineWidth: isFocused ? 1.5 : 1)
        )
